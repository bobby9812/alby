...
wts7.versionNumber=wts7.versionNumber||"8.245";wts7.lastPageShow=0;wts7.lastEvent="load";function wtslog7(alias,db,obj,event_name,event_conversion,event){wts7.alias=alias;try{wtslog7Exe(alias,db,obj,event_name,event_conversion,event);}
catch(e){wtsDebug7('flag 1',e);}}
function wtslog7Exe(alias,db,obj,event_name,event_conversion,event){if(/bot|crawl|google|baidu|bing|msn|teoma|slurp|yandex/i.test(navigator.userAgent)){return;}
if(event){wts7.thisEvent=event.type;}
else{wts7.thisEvent="load";}
if(wts7.thisEvent!=wts7.lastEvent&&Date.now()-wts7.lastPageShow<1500){return;}
if(event_name){if(event){event.preventDefault();}}
if(wts7.origin=='wtslog6_2'){wts7.versionNumber='6.28';}
if(!alias||!db){return;}
var start=Date.now();var web_url="www.web-stat.com";var app_url="app.ardalio.com";var app_name="Web-Stat";if(document.readyState==='loading'){if(typeof document.addEventListener==='function'){document.addEventListener("DOMContentLoaded",function(){wtslog7Exe(alias,db,obj,event_name,event_conversion,event);});return;}
else{document.getElementById('wts'+alias).innerHTML='<a href="https://'+web_url+'"><img alt="'+app_name+' Web Analytics" src="https://'+app_url+'/7/'+db+'/'+alias+'.png"></a>';}}
...
return true;};var gmtimeUnix,timeLastLoad,sessionId,visitorId,nVisits,referer,title,url,checkedOnline,originalRef,allow_cookies,isOwner,isEvent,params,invisible,conversion_number,page_name,group_name,checked_online,qry,cs,s,newSpan,userId,uidsid;checkedOnline="";gmtimeUnix=Math.floor(Date.now()/1000);wts7.origin=wts7.origin||'log7';params=wts7.params||'';invisible=wts7.invisible||'';conversion_number=wts7.conversion_number||'';page_name=wts7.page_name||'';group_name=wts7.group_name||'';referer=wts7.ref||'';if(referer!==''){referer=referer+'::::::';}
if(!wts7.focusEvent){window.addEventListener('focus',function(){wts7.focusEvent=1;start=Date.now();if(Date.now()-start>1800000){wtslog7Exe(alias,db,obj,event_name,event_conversion,event);}});}
if(wts7.params!='editor_load'&&wts7.params!='no_count'){wts7.url='';wts7.scrW=screen.width||window.innerWidth||window.screen.height||0;wts7.scrH=screen.height||window.innerHeight||window.screen.height||0;checked_online='';if(typeof event_name!=='undefined'&&event_name!=''){isEvent='yes';invisible='yes';conversion_number=event_conversion||'';event_name=event_name.replace(/^\\s+|\\s+\$/g,'');event_name=encodeURIComponent(event_name);if(alias=='197785'){page_name=event_name;group_name=event_name;url=document.URL||page_name;title=document.title||page_name;}
else{page_name='event_'+event_name;group_name=event_name;url=event_name;title=event_name;}}
...
var rgx={};rgx.ack=/^1$|^0$/;rgx.tll=/^\d+$/;rgx.sid=/^[\w\-@]+$/;rgx.uid=/.+/;rgx.vid=/^[\w\-@\.]+$/;rgx.bid=/^[\w\-@]+$/;rgx.nv=/^\d+$/;rgx.io=/^1$|^0$/;rgx.lv=/^\d+$/;rgx.or=/^[^\[]+$/;rgx.prf=/^[^\[]+$/;rgx.cty=/.+/;rgx.asn=/^\d+$/;if(wts7.allow_cookies===false){allow_cookies=0;}
...
timeLastLoad=getData7('tll',alias,gmtimeUnix,rgx)||'0';sessionId=getData7('sid',alias,gmtimeUnix,rgx)||'';userId=wts7.user_id||getData7('uid',alias,gmtimeUnix,rgx)||'';visitorId=getData7('vid',alias,gmtimeUnix,rgx)||'';nVisits=getData7('nv',alias,gmtimeUnix,rgx)||0;isOwner=getData7('io',alias,gmtimeUnix,rgx)||'0';if(alias==1024||alias==2064256){console.log("isOwner: "+isOwner+" / allow_cookies: "+allow_cookies);}
if(isOwner!='1'&&isOwner!='0'){wtsDebug7('1/ bad isOwner : '+isOwner);}
originalRef=getData7('or',alias,gmtimeUnix,rgx)||'';if(timeLastLoad!='0'){writeData7('tll_'+alias,timeLastLoad,allow_cookies,315360000,gmtimeUnix,rgx);}
...
if(userId!=''){writeData7('uid_'+alias,userId,allow_cookies,315360000,gmtimeUnix,rgx);}
...
if(userId!=''&&uidsid==sessionId){userId='d_'+userId;}
...
params+'&'+isEvent+'&'+checkedOnline+'&'+userId+'&'+wts7.versionNumber+'&'+allow_cookies+'&'+Math.random();if(event_name){cs=document.createElement("script");cs.type="text/javascript";cs.async="async";cs.src="https://"+app_url+"/count7.pl?"+qry;s=document.getElementsByTagName("script")[0];if(s){s.parentNode.insertBefore(cs,s);}
else{document.head.appendChild(cs);}
if(obj){cs.onload=function(){redirect7(obj);};return false;}}
else{newSpan=document.createElement("span");newSpan.setAttribute("id","wtstimer"+alias);newSpan.style.display="none";document.body.appendChild(newSpan);cs=document.createElement("script");cs.type="text/javascript";cs.async="async";cs.src="https://"+app_url+"/count7.pl?"+qry;s=document.getElementsByTagName("script")[0];if(s){s.parentNode.insertBefore(cs,s);}
else{document.head.appendChild(cs);}
cs.onload=function(){finalProcessing7(alias,db,app_url,gmtimeUnix,params,invisible,wts7,0,obj,event_name,event_conversion,event);};}}
...
if(response.indexOf("::")==-1){loop++;if(loop<100){setTimeout(function(){finalProcessing7(alias,db,app_url,gmtimeUnix,params,invisible,wts7,loop,obj,event_name,event_conversion,event);},100);return;}
return;}
if(!wts7.pageShowEvent){window.addEventListener("pageshow",function(event){wts7.pageShowEvent=1;wtslog7Exe(alias,db,obj,event_name,event_conversion,event);});}
...
function pingPage7(pingId,last_active,delay,ajaxRequest,app_url,alias,db){var currTime,msg;currTime=Math.floor(Date.now()/1000);if(currTime>last_active+delay/1000+1800){return;}
var hasFocus;try{hasFocus=top.document.hasFocus()||document.hasFocus();}
catch(e){}
if(document.visibilityState!="hidden"&&hasFocus){last_active=currTime;delay=delay+1000;if(delay>30000){delay=30000;}
msg="https://"+app_url+"/ping_timer.pl?"+pingId+"&"+delay+"&ck="+Math.random();ajaxRequest.open('GET',msg,true);ajaxRequest.send();setTimeout(function(){pingPage7(pingId,last_active,delay,ajaxRequest,app_url,alias,db);},delay);}
else{setTimeout(function(){pingPage7(pingId,last_active,0,ajaxRequest,app_url,alias,db);},2000);}}
...
function writeData7(id,data,allow_cookies,max_age,now,rgx){var domain,item,expiry;if(!data){return;}
data=data.toString();if(data==='0'||data===''){return;}
...
if(domain){try{document.cookie=id+'='+data+'; domain='+domain+'; path=/; SameSite=None; Secure; Max-Age='+max_age;}
catch(e){wtsDebug7('writeData 3.1 / error writing cookies on '+domain,e);}}
else{try{document.cookie=id+'='+data+'; path=/; SameSite=None; Secure; Max-Age='+max_age;}
